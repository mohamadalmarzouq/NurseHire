...
export default function CaretakerCallsPage() {
  ...
  const handleRespond = async (callId: string, status: 'ACCEPTED' | 'REJECTED') => {
    try {
      const response = await fetch(`/api/calls/${callId}/respond`, {
        method: 'PUT',
      ...
      if (response.ok) {
        ...
        alert(error.error || 'Failed to update call')
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <h1 className="nh-h1">Call Requests</h1>
          <p className="nh-sub">Manage your scheduled video call requests</p>
        </div>
        ...
        {calls.length === 0 ? (
          <div className="nh-card text-center py-12">
            ...
            <p className="text-gray-600">
              You have no video call requests at the moment.
            </p>
          </div>
        ) : (
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="text-lg font-semibold text-gray-900">
                        Call with {call.user?.userProfile?.name || 'User'}
                      </h3>
                      <span
