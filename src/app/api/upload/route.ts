...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!hasCloudinaryUrl && !hasIndividualVars) {
      ...
      return NextResponse.json(
        { error: 'Cloudinary is not configured. Please set either CLOUDINARY_URL or all of: CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, and CLOUDINARY_API_SECRET environment variables.' },
        { status: 500 }
      )
    }
    
    // Log which method is being used (but not the values)
    console.log('Cloudinary environment variables check:', {
      method: hasCloudinaryUrl ? 'CLOUDINARY_URL' : 'Individual variables',
      CLOUDINARY_URL: hasCloudinaryUrl ? '✓ Set' : '✗ Missing',
