...
export async function POST(
  ...
  try {
    ...
    if (callSession.recordingStatus === RecordingStatus.RECORDING) {
      return NextResponse.json({ error: 'Recording already in progress' }, { status: 400 })
    }

    const res = await fetch(
      `https://api.daily.co/v1/rooms/${callSession.dailyRoomName}/recordings/start`,
      {
        method: 'POST',
        headers: {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        { error: errorData.error || 'Failed to start recording' },
        { status: 500 }
      )
    }

    const data = await res.json()
    const recordingId = data?.recording_id || data?.id || null
