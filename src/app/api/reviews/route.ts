...
export async function GET(request: NextRequest) {
  try {
    ...
    if (type === 'given') {
      ...
      if (payload.role === 'CANDIDATE' && includePending) {
    ...
    return NextResponse.json({
      reviews: reviews.map(review => ({
        ...
        giver: {
          id: review.giver.id,
          name: review.giver.userProfile?.name || review.giver.candidateProfile?.name || 'Unknown',
          role: review.giver.role,
        },
        receiver: {
          id: review.receiver.id,
          name: review.receiver.userProfile?.name || review.receiver.candidateProfile?.name || 'Unknown',
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!receiver || !receiver.candidateProfile) {
    ...
    if (existingReview) {
      return NextResponse.json({ error: 'Review already exists for this candidate' }, { status: 400 })
    }
